...
use Zend\Crypt\Hmac as HmacEncryptor;
...
class HmacValidator extends ConstraintValidator
{
    ...
    public function validate($app, Constraint $constraint)
    {
        ...
        $zendCrypt = new HmacEncryptor();
        $hmac = $zendCrypt->compute($app['hmac_key'], 'sha1', $this->buildHmacString($app), true);
    ...
    public function buildHmacString($app)
    {
        $hmacData = array(
